...
from geezram.modules.helper_funcs.extraction import extract_user_and_text
from geezram.modules.helper_funcs.string_handling import extract_time
from geezram.modules.log_channel import gloggable, loggable
...
def ban(update: Update, context: CallbackContext) -> str:
    ...
    if is_user_ban_protected(chat, user_id, member) and user not in DEV_USERS:
        if user_id == OWNER_ID:
            ...
            message.reply_text(
                "Bring an order from DEMONS to fight a Assasin servant."
            )
        elif user_id in TIGERS:
            message.reply_text(
                "Bring an order from WOLVES to fight a Lancer servant."
            )
        elif user_id in WOLVES:
            message.reply_text("TIGERS access make them ban immune!")
        else:
