...
from geezram.__main__ import STATS, TOKEN, USER_INFO
from geezram.modules.sql import SESSION
import geezram.modules.sql.userinfo_sql as sql
...
from geezram.modules.sql.afk_sql import is_afk, set_afk
...
def info(update: Update, context: CallbackContext):
    ...
    if user.id == OWNER_ID:
        ...
        text += "\n\nThe Disaster level of this person is 'DEMONS'."
        disaster_level_present = True
...
def stats(update: Update, context: CallbackContext):
    stats = "<b>Current Gees|RAM Statistics</b>\n" + "\n".join([mod.__stats__() for mod in STATS])
    result = re.sub(r"(\d+)", r"<code>\1</code>", stats)
    result += "\n<b> Powered By Geez|RAM </b>"
    update.effective_message.reply_text(
        result,
        parse_mode=ParseMode.HTML, 
        disable_web_page_preview=True
