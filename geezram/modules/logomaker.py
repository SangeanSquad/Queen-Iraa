...
from geezram import telethn as tbot, ubot2
...
async def logo_gen(event):
    ...
    if event.reply_to_msg_id:
        ...
        async for i in ubot2.iter_messages(
            "@geezramlogo", filter=InputMessagesFilterPhotos
        ):
            pics.append(i)
        id_ = random.choice(pics)
        bg_ = await id_.download_media()
        fpath_ = glob.glob("./geezram/resources/fonts/*")
    ...
    if os.path.exists(font_):
        if not font_.startswith("./geezram/resources/fonts"):
            os.remove(font_)
